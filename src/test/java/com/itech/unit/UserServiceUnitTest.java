...
class UserServiceUnitTest {
  ...
  void createdUserNotExists_whenCreateUser_thenCheckEmailMessageStructure() {
    ...
    when(userRepository.findUserByRole(Role.MANAGER))
        .thenReturn(
            Optional.of(new User("manager", "manager", "manager@manager.ru", Role.MANAGER)));
    ...
    assertThat(titleCaptor.getValue())
        .isEqualTo(String.format(userConfirmationMessageTitleText, anyUserEntity.getUsername()));
    assertThat(messageCaptor.getValue())
        .isEqualTo(String.format(("%s%s"), confirmMessage, "token1"));
  }
  ...
  void givenUsers_andFindUserByUsername_thenCheckAllValues() {
    ...
    assertThat(userService.findUserByUsername("user").getUsername())
        .isEqualTo(anyUser.getUsername());
    assertThat(userService.findUserByUsername("user").getPassword())
  ...
  void givenUser_andActivateUser_thenCheckEmailMessageStructure() {
    ...
    doNothing()
        .when(emailService)
        .sendEmail(anyUser.getEmail(), successfulConfirmationTitle, successfulConfirmationMessage);
